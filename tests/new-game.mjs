...
try{
 for(const viewport of [{width:1280,height:800},{width:320,height:568},{width:844,height:390}]){
  ...
  await click('Deploy expedition');await p.waitForFunction(()=>window.frontierState.started&&window.frontierState.map_id==='expanse');
  ...
  await click('Deploy expedition');await p.waitForFunction(()=>window.frontierState.started&&window.frontierState.map_id==='riverlands');
